...
awk -F'|' 'tolower($7) ~ /novotel/ {print "citycode: "$4,"/ cityname: "$5, "/ hotelname: "$7;}' Extended.csv
...
bc. find . -type f -name 'Hotel*' | while read file; do [[ "$file" =~ \.([A-Z]{3})\. ]] && {  sed -r 's/(.*)/'${BASH_REMATCH[1]}'\1/' $file; };donefind . -type f -name 'Hotel*' | while read file; do [[ "$file" =~ \.([A-Z]{3})\. ]] && {  sed -r 's/(.*)/'${BASH_REMATCH[1]},'\1/' $file; };done